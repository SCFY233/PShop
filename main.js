...
mc.listen("onServerStarted", () => {
    ...
    if (plus.includes("PVip")) {
        imports.PVip.enable = true;
        imports.PVip.getplvipstatus = ll.import("PVip", "get_status")
    } else {
        logger.warn(lang.get("import.error.PVip"));
    }
    if (plus.includes("PMail")) {
        imports.PMail.enable = true;
        imports.PMail.regsyssendp = ll.import("PMail", "regsystem");
        imports.PMail.sendmail = ll.import("PMail", "addnewmail");
        imports.PMail.regsyssendp(info2);
    } else {
        logger.warn(lang.get("import.warn.PMail"));
    }
    if (plus.includes("PLib")) {
        imports.PLib.enable = true;
        imports.PLib.getItem = ll.import("PLib", "item")
    } else {
        logger.error(lang.get("import.error.PLib"));
        setTimeout(() => {
            mc.runcmd("ll unload PShop")
        }, 3000);
    }
    if (plus.includes("FinalTextures")) {
        imports.FinalTextures.enable = true;
        imports.FinalTextures.getTextures = ll.import("FinalTextures", "getTextures")
    } else {
        logger.error(lang.get("import.error.FinalTextures"));
        setTimeout(() => {
            mc.runcmd("ll unload PShop")
        }, 3000);
    }
...
const moneys = {
    ...
    reduce(player, value) {
        switch (moneys.conf.type) {
            case "llmoney":
                money.reduce(player.xuid, Number(value));
                return true;
            case "score":
